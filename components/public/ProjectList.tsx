            ...
            >
                {filtered.map((project) => (
                    <motion.div
                        ...
                        key={project.id}
                        className="group relative flex flex-col overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900/50 hover:border-gray-300 dark:hover:border-gray-700 transition-colors"
                    >
                        ...
                        <div className="flex flex-1 flex-col p-5">
                            ...
                            <div className="mt-4 flex flex-wrap gap-2">
                                {project.techStack.slice(0, 3).map((stack) => (
                                    <span key={stack} className="inline-flex items-center rounded-md bg-gray-100 px-2 py-1 text-xs font-medium text-gray-600 dark:bg-gray-800 dark:text-gray-300">
                                        {stack}
                                    </span>
                                ))}
                                {project.techStack.length > 3 && (
                                    <span className="inline-flex items-center rounded-md bg-gray-50 px-2 py-1 text-xs font-medium text-gray-500 dark:bg-gray-900 dark:text-gray-500">
                                        +{project.techStack.length - 3}
                                    </span>
                                )}
                            </div>
                            ...
                            <div className="mt-4 flex justify-end">
                                <ArrowUpRight className="h-5 w-5 text-gray-300 dark:text-gray-600 group-hover:text-blue-500 transition-colors" />
                            </div>
                        </div>
                    </motion.div>
                ))}
